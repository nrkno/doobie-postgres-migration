      ...
      case _ => // validate that there's only filenames matching our filename schema
        IO.raiseError(
          DoobiePostgresMigrationException(
            s"Found non-matching filename '${file.getAbsolutePath}'. All files in dir: '${migrationsDir.getAbsolutePath}' must match regex: ${MigrationFileRegex}"
          )
        )
    }
    ...
    def getFileMigrationsAndValidateFileCount(eitherUpOrDown: List[MigrationFromFile]): IO[List[Migration]] = {
      ...
      upAndDownsById.traverse { case (id, upsAndDowns) =>
        ...
        if (manyDowns.length <= 1 || manyUps.length <= 1) {
          ...
          IO.raiseError(
            DoobiePostgresMigrationException(
              s"Did not find EXACTLY these files: ${idToUpFilePath(id)} and ${idToDownFilePath(id)} in dir: '${migrationsDir.getAbsolutePath}'"
            )
          )
        }
      }
      ...
      _ <- migrations.traverse[ConnectionIO, Unit] { curr =>
        ...
        existingHashByIdFromDb.get(id) match {
          ...
          case Some(Seq(Migration(`id`, _, _, md5))) =>
            FC.raiseError(
              DoobiePostgresMigrationException(
                s"Wrong hash for '$id'! DB says: '$md5'. Current is: '$fileHash'. Did files: ${idToDownFilePath(id)} and ${idToUpFilePath(id)} change?"
              )
            )
          case Some(alts) =>
            FC.raiseError(
              DoobiePostgresMigrationException(s"Expected to find one or zero pairs with id: '$id', but found: ${alts.mkString(",")}")
            ) // can only happen if id is no longer a PRIMARY KEY
        }
      }
    } yield ()
  }
